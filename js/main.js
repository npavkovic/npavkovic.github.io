...
$id("hamburger").addEventListener("click", (e) => {
	if(document.body.classList.contains("toc-active")) {
		...
		if (dom.activePage?.id) {
			dom.activeBreadcrumb = dom.toc.querySelector(`[data-id="${dom.activePage.dataset.id ||dom.activePage.id }"]`)
...
const enterChapter = new IntersectionObserver((elements, observer) => {
	elements.forEach((element) => {
		if (element.isIntersecting) {
			console.log(`Intersecting Ratio for ${element.target.title}: ${element.intersectionRatio}\nTop: ${element.intersectionRect.top}\nBottom: ${element.intersectionRect.bottom}`);
...
$("page").forEach((page) => {
	...
	siteIndex[page.dataset.category] = siteIndex[page.dataset.category] || { id: '', pages: [] };
...
tocContent.addEventListener("click", (e) => { document.body.classList.remove("toc-active"); });
